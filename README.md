...
2. Текущая версия программы может быть указана через специальный файл (для примера назовём его VERSION 1.0) внутри самого архива, в её интерфейсе и в свойствах бинарных файлов. Внутрь файла VERSION 1.0 можно поместить историю версий в любом удобном формате.
...
4. Само автоматическое обновление происходит по схеме: 1) перевод директории на текущую; 2) удаление всех файлов и директорий в папке за исключением пользовательских; 3) скачивание архива; 4) распаковка архива; 5) удаление архива. Реализации на Windows и Linux отличаются: Linux хранит исполняемый скрипт в оперативной памяти и делает возможным его горячее удаление, в отличие от Windows. Поэтому на Windows необходимо создание второго скрипта, который возьмёт на себя работу после удаления первого. Batch поддержка начиная с Windows 10.
...
## Особенности GRU
1. Универсальность. Не требуется установки никаких отдельных программ для соблюдения всех спецификаций. Принципы системы могут быть совместимы со множеством файловых хранилищ, включая IPNS и Google Drive.
