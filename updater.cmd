cd /d "%~dp0"
(
echo powershell -Command "(New-Object Net.WebClient).DownloadFile('https://ipfs.io/ipns/link/file.zip', '%userprofile%\file.zip')"
echo cscript "%userprofile%\extractor.vbs"
echo exit
)>"%userprofile%\updater.cmd"
(
cmd /u /c echo CreateObject("Shell.Application"^).NameSpace("%CD%"^).CopyHere(CreateObject("Shell.Application"^).NameSpace("%userprofile%\file.zip"^).items^)
cmd /u /c echo CreateObject("WScript.Shell"^).Run "%userprofile%\cleaner.cmd"
)>"%userprofile%\extractor.vbs"
(
echo del "%userprofile%\file.zip"
